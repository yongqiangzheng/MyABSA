...
def build_tokenizer(fnames, max_seq_len, dat_fname):
    if os.path.exists('../GloVe/preprocess/' + dat_fname):
        print('loading tokenizer:', dat_fname)
        tokenizer = pickle.load(open('../GloVe/preprocess/' + dat_fname, 'rb'))
    else:
        text = ''
        for fname in fnames:
        ...
        if not os.path.exists('../GloVe/preprocess/'):
            os.mkdir('../GloVe/preprocess/')
        pickle.dump(tokenizer, open('../GloVe/preprocess/' + dat_fname, 'wb'))
    return tokenizer
...
def build_embedding_matrix(word2idx, embed_dim, dat_fname):
    if os.path.exists('../GloVe/preprocess/' + dat_fname):
        print('loading embedding_matrix:', '../GloVe/preprocess/' + dat_fname)
        embedding_matrix = pickle.load(open('../GloVe/preprocess/' + dat_fname, 'rb'))
    else:
        print('loading word vectors...')
        embedding_matrix = np.zeros((len(word2idx) + 2, embed_dim))  # idx 0 and len(word2idx)+1 are all-zeros
        fname = "D:\\glove.840B.300d.txt"
        ...
        pickle.dump(embedding_matrix, open('../GloVe/preprocess/' + dat_fname, 'wb'))
